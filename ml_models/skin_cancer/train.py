...
csv_path = os.path.join(base_dir, "datasets", "skin_cancer", "HAM10000_metadata.csv")
part1_dir = os.path.join(base_dir, "datasets", "skin_cancer", "HAM10000_images_part1")
part2_dir = os.path.join(base_dir, "datasets", "skin_cancer", "HAM10000_images_part2")
model_save_dir = os.path.join(base_dir, "ml_models", "skin_cancer")
model_save_path = os.path.join(model_save_dir, "skin_cancer_model.pth")
...
if not os.path.exists(csv_path):
    raise FileNotFoundError(f"HAM10000 metadata CSV not found at: {csv_path}")

df = pd.read_csv(csv_path)

# Map diagnosis to binary label (dx == 'mel' is malignant melanoma -> 1, all others -> 0)
df['label'] = df['dx'].apply(lambda x: 1 if x == 'mel' else 0)

# Helper function to find absolute image path checking both subdirectories
...
# Add absolute image_path column
df['image_path'] = df['image_id'].apply(get_image_path)

# Drop rows where image file could not be found
df = df.dropna(subset=['image_path'])
print(f"Total valid image samples found: {len(df)}")
